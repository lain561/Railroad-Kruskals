...
def compilerunfile():
    ...
    #javac compile command the executable will be AddressBookTree.class
    
    javac = "javac " + cwd + "/RailroadDriver.java" #compile solution file command and generate class file

    proc = subprocess.run([javac], capture_output=True, text = True, shell = True)
    

    
    java = "java RailroadDriver"
...
def main():
    ...
    subprocess.run(["rm RailroadDriver.class"], capture_output=True, text = True, shell = True)
    subprocess.run(["rm Railroad.class"], capture_output=True, text = True, shell = True)
    
    #remove extra .class files
    for subdir, dirs, files in os.walk(os.getcwd()):
        for myfile in files:
            if(myfile[-6:] == ".class"):
                subprocess.run(["rm " + myfile], capture_output=True, text = True, shell = True)
